...
async def decode_base64(
    value: str = Path(
        ...
        openapi_examples={'awesome': {'summary': 'A awesome base64 string', 'value': AWESOME_BASE64ENCODED}},
    ),
):
...
async def delay_response(*, delay: float = Path(..., ge=0, le=10, description='delay seconds'), request: Request):
...
async def drip(
    ...
    numbytes: int = Query(
        default=10,
        gt=0,
        lt=10 * 1024 * 1024,  # set 10MB limit
        description='The number of bytes to respond with',
    ),
    code: int = Query(default=200, title='response code', description='The response code that will be returned'),
...
async def link_page(
    *,
    n: int = Path(
        ...,
        ge=1,
        le=200,  # limit to between 1 and 200 links
...
async def range_request(
    numbytes: int = Path(..., ge=1, le=100 * 1024, description='number of bytes must be in the range (0, 102400]'),
):
...
@router.get('/uuid', response_class=JSONResponse, summary='Return a UUID4.', response_description='A UUID4.')
async def get_uuid4():
